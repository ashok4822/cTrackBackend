...
export class AIChatContextBuilder {
        ...
        if (containers.length === 0) {
            ...
            for (const c of containers) {
                lines.push(
                    ...
                    (c.damaged ? ` | ⚠ DAMAGED${c.damageDetails ? `: ${c.damageDetails}` : ""}` : "") +
        ...
        if (requests.length === 0) {
            ...
            for (const r of requests) {
                const line = [
                    ...
                    r.containerSize ? `  Size: ${r.containerSize} ${r.containerType || ""}` : "",
                    ...
                    r.specialInstructions ? `  Instructions: ${r.specialInstructions}` : "",
                    r.isHazardous ? `  ⚠ Hazardous Material` : "",
                    r.createdAt ? `  Submitted: ${fmt(r.createdAt)}` : "",
                    (r.checkpoints && r.checkpoints.length > 0)
                        ? `  Checkpoints: ${r.checkpoints.map((cp: any) => `${cp.location} (${cp.status})`).join(" → ")}`
                        : "",
                ].filter(Boolean).join("\n");
                lines.push(line);
            }
        ...
        if (bills.length === 0) {
            ...
            for (const b of bills) {
                lines.push(
                    ...
                    ` | Due: ${fmt(b.dueDate)}` +
                    (b.paidAt ? ` | Paid On: ${fmt(b.paidAt)} via ${b.paymentMethod || "N/A"}` : "") +
    ...
    // ─────────────────────────────────────────
    static async buildGeneralContext(customerId: string, userId: string): Promise<string> {
        const containerRepo = new ContainerRepository();
        const requestRepo = new ContainerRequestRepository();
        const billRepo = new BillRepository();
        const pdaRepo = new PDARepository();
        ...
        const activeContainers = containers.filter((c) =>
            ["gate-in", "in-yard", "in-transit", "at-port", "at-factory"].includes(c.status)
        );
        const pendingRequests = requests.filter((r: any) =>
            ["pending", "approved"].includes(r.status)
        );
        const overdueOrPendingBills = bills.filter((b) =>
        ...
        lines.push(`Stuffing Requests: ${requests.filter((r: any) => r.type === "stuffing").length}`);
        lines.push(`Destuffing Requests: ${requests.filter((r: any) => r.type === "destuffing").length}`);
        lines.push(``);
        lines.push(`── BILLING ──`);
        lines.push(`Total Bills: ${bills.length}`);
