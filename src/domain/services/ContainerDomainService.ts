...
import { IContainerDomainService, GateInContainerData } from "./IContainerDomainService";
...
export class ContainerDomainService implements IContainerDomainService {
  ...
  async processGateIn(data: GateInContainerData, existingContainer: Container | null): Promise<Container> {
    ...
    if (!existingContainer) {
      ...
      return new Container(
        ...
        data.size || existingContainer.size,
        data.containerType || existingContainer.type,
        "gate-in",
        data.shippingLine || existingContainer.shippingLine,
        data.empty ?? existingContainer.empty,
        data.movementType || existingContainer.movementType,
        data.customer || existingContainer.customer,
        customerName || existingContainer.customerName,
        existingContainer.yardLocation,
        new Date(),
        undefined,
        undefined,
        data.weight ?? existingContainer.weight,
        data.cargoWeight ?? existingContainer.cargoWeight,
        data.cargoDescription ?? existingContainer.cargoDescription,
        data.hazardousClassification ?? existingContainer.hazardousClassification,
        data.sealNumber ?? existingContainer.sealNumber,
        existingContainer.damaged,
        existingContainer.damageDetails,
        existingContainer.blacklisted,
        data.cargoCategory || existingContainer.cargoCategory,
  ...
  async processGateOut(container: Container): Promise<Container> {
    return new Container(
      container.id,
