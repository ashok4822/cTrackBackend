...
exports.BillingDomainService = void 0;
const BillingMapper_1 = require("../../application/mappers/BillingMapper");
class BillingDomainService {
    containerRepository;
    billRepository;
    activityRepository;
    chargeRepository;
    constructor(containerRepository, billRepository, activityRepository, chargeRepository) {
        this.containerRepository = containerRepository;
        this.billRepository = billRepository;
        this.activityRepository = activityRepository;
        this.chargeRepository = chargeRepository;
    }
    async generateBillForRequest(request, billIdentifier) {
        // 1. Find Container
        ...
        if (lineItems.length > 0 && container.id) {
            ...
            if (pendingBill) {
                ...
                const updatedBill = BillingMapper_1.BillingMapper.appendLineItems(pendingBill, lineItems, billIdentifier, request.type);
                await this.billRepository.save(updatedBill);
            }
            else {
                const bill = BillingMapper_1.BillingMapper.createForRequest(request.type === "stuffing" ? "STUF" : "DEST", containerNumber, container.shippingLine, container.id, request.customerId, lineItems, totalAmount, request.type, billIdentifier);
                await this.billRepository.save(bill);
            }
        }
    }
    async findApplicableCharge(activityId, size, type, cargoCategoryId) {
        if (cargoCategoryId) {
...
exports.BillingDomainService = BillingDomainService;
//# sourceMappingURL=BillingDomainService.js.map