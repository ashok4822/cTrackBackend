...
class DashboardMapper {
    static toResponseDto(data) {
        ...
        const recentActivities = data.recentActivitiesRaw
            ...
            if (activity.includes("gate"))
                type = "gate";
            if (activity.includes("payment"))
                type = "payment";
            if (activity.includes("survey"))
                type = "survey";
            if (activity.includes("approval"))
                type = "approval";
            const containerNumber = history.containerId?.containerNumber || "Unknown";
            return {
                id: history.id,
        ...
        if (data.pendingRequestsCount > 0) {
            recentAlerts.push({
                id: "pending-requests",
                type: "warning",
                title: ResponseMessage_1.ResponseMessage.PENDING_APPROVALS_TITLE,
        ...
        data.damagedContainers.forEach((container) => {
            recentAlerts.push({
                ...
                type: "error",
                title: ResponseMessage_1.ResponseMessage.DAMAGED_CONTAINER_TITLE,
        ...
        data.equipmentIssues.forEach((eq) => {
            recentAlerts.push({
                ...
                title: eq.status === "down" ? ResponseMessage_1.ResponseMessage.EQUIPMENT_DOWN_TITLE : ResponseMessage_1.ResponseMessage.EQUIPMENT_MAINTENANCE_TITLE,
...
exports.DashboardMapper = DashboardMapper;
